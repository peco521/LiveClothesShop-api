...
from app.modules.seguridad_accesos.cu07_clientes.schemas.cliente import (
    ClienteDetalle, ClienteEditar, ClienteEstadoCuenta, ClientesListado,
)
...
def list_clients(request: Request, offset: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100),
                 q: str = Query("", max_length=100), activo: bool | None = None,
                 db: Session = Depends(get_db)):
    return cliente.list_clients(db, request.app.state.settings, offset=offset, limit=limit, q=q.strip(), activo=activo)
...
def set_state(idUsuario: str, data: ClienteEstadoCuenta, request: Request, db: Session = Depends(get_db),
              identity=Depends(cu07_identity)):
    return cliente.set_state(db, idUsuario, data, request.app.state.settings, identity.usuario.idUsuario, peer(request))
